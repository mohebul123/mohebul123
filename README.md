<p align="center">
  <img src="banner.png" width="100%" alt="Md Mohebul Islam - Full Stack Developer Banner" />
</p>

<h1 align="center">Md Mohebul Islam</h1>
<p align="center"><strong>Full-Stack Web Developer | Software Engineer</strong></p>

<p align="center">
  <a href="https://linkedin.com/in/mohebul123" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>&nbsp;&nbsp;
  <a href="https://github.com/mohebul123" target="_blank"><img src="https://img.shields.io/badge/GitHub-101010?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" /></a>&nbsp;&nbsp;
  <a href="https://facebook.com/mohebul123" target="_blank"><img src="https://img.shields.io/badge/Facebook-1877F2?style=for-the-badge&logo=facebook&logoColor=white" alt="Facebook" /></a>&nbsp;&nbsp;
  <a href="https://twitter.com/mohebul123" target="_blank"><img src="https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white" alt="Twitter" /></a>&nbsp;&nbsp;
  <a href="mailto:mohebulhassan123@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>
</p>

---

### 👨‍💻 ABOUT ME
I am a passionate software engineer dedicated to building high-performance digital solutions and robust backend ecosystems. With a solid foundation from my B.Sc. in Computer Science & Engineering, I focus on transforming complex system logic into smooth, secure, and maintainable code architectures.

---

### 🔭 CURRENT ACTIVITIES
* 🚀 **Exploring:** Deepening knowledge in Next.js Server Components and backend systems orchestration.
* 🛠️ **Working On:** Building **FoodHub** — an enterprise-grade multi-vendor food delivery ecosystem.
* ⚙️ **Integration workflows:** Implementing production Stripe subscription frameworks and setting up automated Vercel telemetry webhooks.

---

### 🎓 ACADEMIC & ARCHITECTURAL FOUNDATIONS
*As a Computer Science graduate, my development approach is guided by fundamental engineering paradigms and structured software design principles learned throughout my academic tenure:*

*   🔐 **Software Security & Trust:** Deep understanding of the **CIA Triad** (Confidentiality, Integrity, Availability), secure identity management, non-repudiation systems, and robust multi-factor authentication protocols.
*   📐 **Requirement Engineering & Modeling:** Skilled in analyzing business rules, producing structural UML diagrams, and executing comprehensive requirements validation matrices before writing the first line of code.
*   🔄 **SDLC & Methodology Operations:** Practical grasp of modern software development lifecycles, adapting agile practices (Scrum/Sprints) and structured Waterfall approaches based on project complexity to control deployment and continuous maintenance lifecycles.
*   🧪 **Testing & Quality Assurance:** Capable of designing systematic verification plans utilizing both **Black-Box** and **White-Box testing** methodologies to guarantee component-level and end-to-end integration stability.
*   🎨 **UI/UX Design Principles:** Applying user-centric design heuristics to build intuitive, responsive, and seamless layouts that perfectly bridge backend logic with frontend ease-of-use.

---

### 🛠️ TECH STACK & TOOLS

<table>
  <tr>
    <td align="center" width="33%">
      <strong>Frontend Layer</strong><br/><br/>
      <img src="https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB" /><br/>
      <img src="https://img.shields.io/badge/TypeScript-007ACC?style=flat-square&logo=typescript&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat-square&logo=tailwind-css&logoColor=white" />
    </td>
    <td align="center" width="33%">
      <strong>Backend & Runtime</strong><br/><br/>
      <img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/Express.js-000000?style=flat-square&logo=express&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/PostgreSQL-336791?style=flat-square&logo=postgresql&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/Prisma_ORM-2D3748?style=flat-square&logo=prisma&logoColor=white" />
    </td>
    <td align="center" width="33%">
      <strong>Security & Dev Tools</strong><br/><br/>
      <img src="https://img.shields.io/badge/JWT-black?style=flat-square&logo=json-web-tokens&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/Vercel-000000?style=flat-square&logo=vercel&logoColor=white" /><br/>
      <img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white" />
    </td>
  </tr>
</table>

---

### 📂 SELECTED PROJECTS
*Below are some of my featured full-stack applications that reflect my architectural and design methodologies:*

#### 🍔 [FoodHub — Multi-Vendor Food Delivery Platform](https://github.com/mohebul123/L2B6A4_foodHub-FrontEnd)
*   **Description:** A complete ecosystem with dynamic customer flows, role-based dashboards, and integrated payment models.
*   **Tech Stack:** Next.js, Node.js, Express, PostgreSQL, Prisma, Better Auth.
*   **Deployment:** Live Site: [foodhub-frontend-red.vercel.app](https://foodhub-frontend-red.vercel.app)

#### ⚡ [Eco-Spark — Innovation Management System](https://github.com/mohebul123/eco-spark-backend)
*   **Description:** High-performance, secure backend architecture optimized for idea tracking, discussion layers, and premium access plans.
*   **Tech Stack:** TypeScript, Express.js, Prisma ORM, PostgreSQL, Zod Validation.
*   **Deployment:** Live Site: [eco-spark-demo.render.com](https://eco-spark-demo.render.com)

#### 🚗 [Vehicle Rental Engine — Enterprise API Platform](https://github.com/mohebul123/VehicleRental-System-BackendAPI-.git)
*   **Description:** Secure database transaction-focused backend system that processes reservations, late-return cost algorithms, and roles control.
*   **Tech Stack:** Node.js, TypeScript, Express.js, PostgreSQL, JWT.
*   **Deployment:** Live Site: [vehiclerentalsystem-seven.vercel.app](https://vehiclerentalsystem-seven.vercel.app/)

---

### 📊 ENGINE TELEMETRY & STATS

<p align="center">
  <img src="https://github-readme-stats-git-master-anuraghazras-projects.vercel.app/api?username=mohebul123&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" width="48%" alt="GitHub Metrics" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=mohebul123&theme=tokyonight&hide_border=true" width="48%" alt="GitHub Streak" />
</p>
